...
typedef CredentialSelector = Future<Credential?> Function(List<Credential> credentials);
...
class Authenticator {
  ...
  static Future<Assertion> handleGetAssertion(GetAssertionOptions options,
          {var localizationOptions =
              const AuthenticationLocalizationOptions(), CredentialSelector? credentialSelector}) =>
  ...
  Future<Assertion> getAssertion(GetAssertionOptions options,
      {var localizationOptions =
          const AuthenticationLocalizationOptions(), CredentialSelector? credentialSelector}) async {
    ...
    if (credentials.length == 1) {
      ...
      if(credentialSelector != null){
        final credential = await credentialSelector!.call(credentials);
        if(credential != null) {
          selectedCredential = credential;
        } else {
          final message = 'No credentials selected for rpId: ${options.rpId} cause user canceled';
          _logger.w(message);
          throw GetAssertionException(message);
        }
      } else {
